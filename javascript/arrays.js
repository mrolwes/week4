...
//colorList.shift(); //remove the first elements
...
//create a new array (list) to hold the purchased imags
var purchasedImages =[];

//var myThumbDiv = document.getElementById("thumbs");
//attach a click event to the DIVwith id="thumbs"
myThumbDiv.addEventListener("click", onImageClick, false);

//the function handler
function onImageClick(evt)
  {
    var onlyPath = evt.target.getAttribute("src"); //Attribute = path
  
    var imageIndex = imageListA.indexOf(onlyPath);
  
     //get the element yo clicked on OUT of the imageListA array and save the clcked 
     //image path to a variable so we cn use it later
  
    var selectedImage = imageListA.splice(imageIndex, 1);
  
     //add this imae path to the purrchasedImage array..
  ...
  function update_shopping_cart()
  {
    ...
    //create an output variable so we can bukd it up over the following lines
    var output = "";
  
    //create a for loop
    for(var i=0; i <= purchasedImages.length -1; i++ )
    
    //use a -1 after imaeList.length so the condition is always one less (to match the index number)
    {
